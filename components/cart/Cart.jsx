...
const Cart = () => {
  ...
  const handleCheckout = async() => {
     ...
     const {data} = await axios.post('https://sneaker-store-six.vercel.app/api/checkout', {lineItems})
  ...
  return (
    <div className="min-w-[275px] h-full px-3 py-6 bg-white text-[#333] rounded-lg shadow-lg cursor-pointer">
      <div>
        <h2 className="text-center text-2xl text-emerald-500 font-bold">Cart Items</h2>
        <div className="max-h-[225px] overflow-auto flex flex-col gap-8 my-8">
          ...
          ) : <span className="text-gray-500 ml-2">Your cart is empty!</span>}
        </div>
        <span className="inline-block">Total: <span>${cartItems.reduce((a, b) => a + b.price * b.quantity, 0)}</span></span>
        <span className="block max-w-max mt-8 px-6 py-1 bg-emerald-500 text-[#efefef] rounded-lg" onClick={handleCheckout}>Checkout</span>
      </div>
    </div>
  )
}
